...
a zip archive carrying the binary and every surface artifact together:

| Entry | What | Source RFC |
|---|---|---|
| `rut.toml` | manifest — format, versions, module specifier | §2 here |
| `mod.rutc` | module binary with bodies | 0033 §1 |
...
- `mod.rutc.map` travels by default when the binary was built with names/
  spans; `--release` strip policy (RFC 0033 §1, 0036 §4) governs both the
  binary's `symbols` and whether the sidecar carries anything at all.
...
surface and binary and is unchanged by bundling; per-entry hashes and
signatures are OQ-2.

## 3. Deterministic packing

Same source + same rutc ⇒ byte-identical `.rutbundle`: fixed entry order
(`rut.toml`, binary, `.d.rut`, `.d.ir`, map), fixed timestamps, STORE (no
...
pinning for large binaries) = OQ-4.
...
- **Bodies** (`extern` linkage, RFC 0029 §6) come from the bundled
  `mod.rutc`, verified (RFC 0033 §2) and linked like any binary; the
  decl-digest compare runs against the bundled DeclIr, so a repacked
  surface that disagrees with the binary fails link, not runtime.
- The bundled `mod.rutc.map` serves `vm.symbolicate` fallback for stripped
  binaries (RFC 0036 §3 path 2) — read from the zip, not the filesystem.
...
- OQ-4: compression — STORE for determinism in v1; pinned DEFLATE for
  large binaries is a layout change (bumps `format_version`).
- OQ-5: does `rutc pack` also accept a `.d.rut`-only input (a surface
  bundle with no binary, for host-decl distribution)? Nothing in §5 depends
  on the binary being present; defer until a host package wants it.
