...
(`Rect.new(..)`), the class-private `Self { .. }` literal, `async`
class methods, sealing without constructors, explicit `self` receivers.
Methods live in impl blocks (RFC 0012 §4) — the class body is fields only.
...
- **Also a shared cell.** A bare `Circle` is a heap cell handle like every
  ...
  (RFC 0011). Reflection:
  a class is walkable **iff** it has `impl reflect.Reflectable for C`
...
- **`async` class methods are allowed** — same function, `await` in
...
- **The receiver is explicit.** An instance method spells its receiver as
  ...
  signature and greppable — the same explicitness rule as `async`
  (RFC 0018 §2); there is no separate "static" method form. Class methods never take
