...
pub struct MyMap {                              // NOT generic — see below
    inner: HashMap<TraitHandle, RutValue>,      // K: fat ref, V: erased
}
    ...
    -> Result<ClassTable, Trap>                 // once per instantiation
{
    ...
    ClassTable::new::<MyMap>(args)
        ...
        .method("get",  |ctx, this: &MyMap, k: TraitHandle|
...
- Builtin types flow back natively — a method may return `Option<V>` or
  ...
  consume trait-object refs there, RFC 0012 §3); rut cannot tell it wasn't
  written in rut.
- Hashing/`eq` on a user type may be **rut code**, reached through the
  trait-object vtable — a method call that re-enters the VM (`VmCtx`,
