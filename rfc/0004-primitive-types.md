...
- `Array<f32>` is a flat `Vec<f32>` behind a header — no per-element boxing,
  no per-element refcount traffic (RFC 0016 §4). `Array<Point>` (dataclass or
...
(RFC 0011). Builtin heap values (arrays, strings, bytes) are handles — they
were born shared. There is no `&`/`*` syntax anywhere.
