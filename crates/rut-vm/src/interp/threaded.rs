...
impl Machine for Vm {
    ...
    fn op_arr_get_ref(&mut self, op: &Op, regs: *mut Slot, pc: u32) -> Result<Flow<Value>, Trap> {
        let Op::ArrGetRef { dst, arr, idx, ty } = op else {
            unreachable_op!("op_arr_get_ref: unexpected op")
        };
        let i = unsafe { (*regs.add(*idx as usize)).i };
