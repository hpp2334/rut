...
impl<'a, 'b> FnCompiler<'a, 'b> {
    ...
    pub(crate) fn compile_shortcircuit(&mut self, op: crate::ast::BinOp, lhs: NodeHandle<AnyExpr>, rhs: NodeHandle<AnyExpr>, sp: crate::span::Span) -> TcResult<TypeId> {
        ...
        match op {
            crate::ast::BinOp::And => self.br(lreg, l_end, l_short),
            _ => self.br(lreg, l_short, l_end),
        }
        self.bind(l_short);
        let short_reg = self.new_reg(TY_BOOL);
        let is_and = op == crate::ast::BinOp::And;
    ...
    pub(crate) fn check_formattable(&mut self, ty: TypeId, sp: crate::span::Span) -> TcResult<()> {
