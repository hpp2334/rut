// kmer-view.js — the JS twin of kmer-view/main.rut (the strings-round1
// phase-2 row): the identical k-mer counting on `new Map()` with
// string keys. The rut row changes only the KEY SPELLING (the sv
// range path — keys cross as borrowed `(seq, i, k)` byte windows, no
// key cell minted), never the op sequence, so the JS twin is
// nmap-knucleotide.js's materialized computation UNCHANGED — JS has
// no borrowed-range key, and the parity IS the gate: same work, same
// checksum (the pinned 2198604). The sequence is generated with
// fasta's LCG (the harness takes no stdin — the same adaptation both
// sides). Scale: seq = 200000 letters.
