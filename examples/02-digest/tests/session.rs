...
fn official_vectors() {
    ...
    for (msg, want) in [
        ...
        assert_eq!(rut_hex(&mut vm, "md5", msg.as_bytes()), want, "md5({msg:?})");
    }
    ...
    assert_eq!(rut_hex(&mut vm, "sha1", b""), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
    assert_eq!(rut_hex(&mut vm, "sha256", b""), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
    assert_eq!(rut_hex(&mut vm, "sha512", b""), "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e");
    assert_eq!(rut_hex(&mut vm, "sha1", b"abc"), "a9993e364706816aba3e25717850c26c9cd0d89d");
    assert_eq!(rut_hex(&mut vm, "sha256", b"abc"), "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
    assert_eq!(rut_hex(&mut vm, "sha512", b"abc"), "ddaf35a193617abacc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a2192992a274fc1a836ba3c23a3feebbd454d4423643ce80e2a9ac94fa54ca49f");
    assert_eq!(rut_hex(&mut vm, "sha1", two_block.as_bytes()), "84983e441c3bd26ebaae4aa1f95129e5e54670f1");
    assert_eq!(rut_hex(&mut vm, "sha256", two_block.as_bytes()), "248d6a61d20638b8e5c026930c3e6039a33ce45964ff2167f6ecedd419db06c1");
    assert_eq!(rut_hex(&mut vm, "sha512", sha512_two_block.as_bytes()), "8e959b75dae313da8cf4f72814fc143f8f7779c6eb9f7fa17299aeadb6889018501d289e4900f7e4331b99dec4b5433ac7d329eeb6dd26545e96e55b874be909");
}
...
fn crate_cross_check_on_padding_edges() {
    ...
    for (i, &n) in lens.iter().enumerate() {
        ...
        assert_eq!(rut_hex(&mut vm, "md5", &data), md5_hex(&data), "md5 len {n}");
        assert_eq!(rut_hex(&mut vm, "sha1", &data), sha1_hex(&data), "sha1 len {n}");
        assert_eq!(rut_hex(&mut vm, "sha256", &data), sha256_hex(&data), "sha256 len {n}");
        assert_eq!(rut_hex(&mut vm, "sha512", &data), sha512_hex(&data), "sha512 len {n}");
        // base64 both directions, both alphabets
        for url in [false, true] {
            let enc = as_str(vm.call("b64_enc", &[Value::Bytes(data.clone()), Value::Bool(url)]).unwrap());
            let want = if url {
...
fn base64_rfc4648_and_errors() {
    ...
    let std = as_str(vm.call("b64_enc", &[Value::Bytes(hi.to_vec()), Value::Bool(false)]).unwrap());
    let url = as_str(vm.call("b64_enc", &[Value::Bytes(hi.to_vec()), Value::Bool(true)]).unwrap());
    ...
    assert_ne!(std, url);
    // errors
    assert!(as_err_str(vm.call("b64_dec", &[Value::Str("!*".into()), Value::Bool(false)]).unwrap()).contains("invalid"));
    assert!(as_err_str(vm.call("b64_dec", &[Value::Str("Zg==Zg==".into()), Value::Bool(false)]).unwrap()).contains("after padding"));
}
...
fn hex_roundtrip_and_errors() {
    ...
    assert!(as_err_str(vm.call("hex_dec", &[Value::Str("zz".into())]).unwrap()).contains("invalid"));
    assert!(as_err_str(vm.call("hex_dec", &[Value::Str("abc".into())]).unwrap()).contains("odd"));
}
...
fn hash_key_vectors_and_cross_check() {
    ...
    // catalogue anchors
    let num = |v: Value| as_u64(v) as u32;
    assert_eq!(num(vm.call("crc32", &[Value::Bytes(Vec::new())]).unwrap()), 0);
    assert_eq!(num(vm.call("crc32", &[Value::Bytes(b"123456789".to_vec())]).unwrap()), 0xCBF43926);
    assert_eq!(num(vm.call("crc32", &[Value::Bytes(b"The quick brown fox jumps over the lazy dog".to_vec())]).unwrap()), 0x414FA339);
    assert_eq!(num(vm.call("fnv1a32", &[Value::Bytes(Vec::new())]).unwrap()), 0x811C9DC5);
    assert_eq!(num(vm.call("fnv1a32", &[Value::Bytes(b"a".to_vec())]).unwrap()), 0xE40C292C);
    assert_eq!(num(vm.call("fnv1a32", &[Value::Bytes(b"foobar".to_vec())]).unwrap()), 0xBF9CF968);
    assert_eq!(as_u64(vm.call("fnv1a64", &[Value::Bytes(Vec::new())]).unwrap()), 0xCBF29CE484222325);
    assert_eq!(as_u64(vm.call("fnv1a64", &[Value::Bytes(b"a".to_vec())]).unwrap()), 0xAF63DC4C8601EC8C);
    assert_eq!(as_u64(vm.call("fnv1a64", &[Value::Bytes(b"foobar".to_vec())]).unwrap()), 0x85944171F73967E8);
    assert_eq!(as_u64(vm.call("djb2", &[Value::Bytes(Vec::new())]).unwrap()), 5381);
    assert_eq!(as_u64(vm.call("djb2", &[Value::Bytes(b"a".to_vec())]).unwrap()), 5381 * 33 + 97);
    assert_eq!(as_u64(vm.call("sdbm", &[Value::Bytes(b"a".to_vec())]).unwrap()), 97);
    ...
    for i in 0..20usize {
        ...
        assert_eq!(num(vm.call("crc32", &[Value::Bytes(data.clone())]).unwrap()), crc32fast::hash(&data), "crc32 #{i}");
        assert_eq!(num(vm.call("fnv1a32", &[Value::Bytes(data.clone())]).unwrap()), fnv32_ref(&data), "fnv1a32 #{i}");
        assert_eq!(as_u64(vm.call("fnv1a64", &[Value::Bytes(data.clone())]).unwrap()), fnv64_ref(&data), "fnv1a64 #{i}");
        assert_eq!(as_u64(vm.call("djb2", &[Value::Bytes(data.clone())]).unwrap()), djb2_ref(&data), "djb2 #{i}");
        assert_eq!(as_u64(vm.call("sdbm", &[Value::Bytes(data.clone())]).unwrap()), sdbm_ref(&data), "sdbm #{i}");
    }
}
...
fn json_codec_against_serde() {
    ...
    let doc = match vm.call("sample_doc", &[]).unwrap() {
        Value::Opaque(c) => c,
        v => unreachable!("{v:?}"),
    };
    let enc = as_str(vm.call("json_enc", &[Value::Opaque(doc)]).unwrap());
...
fn dispatcher_matches_direct_entries() {
    ...
    for algo in ["md5", "sha1", "sha256", "sha512"] {
        let via_disp = rut_hex(&mut vm, algo, &data);
        let direct = match vm.call(algo, &[Value::Bytes(data.clone())]).unwrap() {
            Value::Bytes(v) => hex(&v),
            v => unreachable!("{v:?}"),
        };
        assert_eq!(via_disp, direct, "{algo}");
    }
    let err = as_err_str(vm.call("digest", &[Value::Str("md4".into()), Value::Bytes(b"x".to_vec())]).unwrap());
...
fn stress_8k_under_raised_budgets() {
    ...
    assert_eq!(rut_hex(&mut vm, "md5", &data), md5_hex(&data));
    assert_eq!(rut_hex(&mut vm, "sha256", &data), sha256_hex(&data));
    assert_eq!(rut_hex(&mut vm, "sha512", &data), sha512_hex(&data));
...
fn wrong_kind_arguments_trap_cleanly() {
    let mut vm = session(1_000_000, 8 * 1024 * 1024);
    let err = vm.call("crc32", &[Value::Str("not bytes".into())]).unwrap_err();
    assert!(err.msg.contains("argument"), "{err:?}");
    let err = vm.call("hex_dec", &[Value::Bytes(vec![1])]).unwrap_err();
    assert!(err.msg.contains("argument"), "{err:?}");
}
